...
public class TabelaTarifariaController {
    ...
    @GetMapping
    public ResponseEntity<List<TabelaTarifariaResponseDTO>> getAllTabelasTarifarias() {
        List<TabelaTarifaria> tabelas = tabelaTarifariaService.getAllTabelasTarifarias();
        return ResponseEntity.ok(tabelas.stream().map(tabelaTarifariaMapper::convertToResponseDTO).toList());
    }
